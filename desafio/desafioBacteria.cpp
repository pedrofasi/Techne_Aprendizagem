...
int main(){
    ...
    while(true){
        ...
        for(int i =0; i< 4 ; i++){
            scanf(" %d", &aux);
            total += aux;
            bacterias.push_back(total);
        }
        int cont = 0;
        for(int i = 4; i< N ; i++){
            if(cont == 0){
                mortos = bacterias[cont];
                bacterias.push_back(bacterias[i-1]*2 - (mortos));
                cont++;
            }else{
                mortos = bacterias[cont] - bacterias[cont-1];
                bacterias.push_back(bacterias[i-1]*2 - (mortos));
                cont++;
            }
        }
        printf("%d\n",bacterias[bacterias.size()-1]);
    }
